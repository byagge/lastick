...
class ExtrusionReportView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            # Расчёт заработка для сдельной оплаты: ставка берётся из персональной ставки сотрудника
            # или, если она не задана, из первой активной услуги цеха.
            earnings = Decimal("0")
            if getattr(user, "payment_type", None) == User.PaymentType.VARIABLE:
                # Персональная ставка за 1 кг
...
class PackagingReportView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            # Заработок для сдельной оплаты: ставка берётся из персональной ставки сотрудника
            # или, если она не задана, из первой активной услуги цеха.
            earnings = Decimal("0")
            if getattr(user, "payment_type", None) == User.PaymentType.VARIABLE:
                user_rate = getattr(user, "piecework_rate", None)
